...
const AddFabricDetail = ({
  ...
  const [colorQuantityPayload, setColorQuantityPayload] = useState(
    initialColorQuantityState
  );
  ...
  const onHandleAddColorQuantity = () => {
    ...
    if (!totalOrderQuantity) {
      alert(
        "You haven't entered total order quantity, Please specify total order quantity first"
      );
      return;
    }
    ...
    if (totalOrderQuantity) {
      if (getValues("quantity")?.length) {
        ...
        if (totalQuantity > totalOrderQuantity) {
          alert(
            "Sum of all quantity for a single fabric shouldn't exceed Total Order Quantity"
          );
          return;
        }
      } else {
        ...
        <div className={"w-full grid grid-cols-2 gap-5 md:gap-8 p-5"}>
              ...
              <FormItem className={"col-span-full flex flex-col gap-1.5"}>
                ...
                <FormControl>
                  <div className={"inline-flex flex-wrap items-center gap-3"}>
                    <OptionSelector
                      options={fabricNameList?.filter(
                        (fabricItem) => fabricItem?.key !== value
                      )}
                      searchable={true}
                      selectedOption={
                        <Button
            ...
            render={({ field: { value, onChange } }) => (
              <FormItem className={"col-span-full flex flex-col gap-1.5"}>
                <div className={"flex items-center justify-between gap-3"}>
                  ...
                  <OptionSelector
                    options={Object.values(processNameEnums)?.filter(
                      (processItem) => !value?.includes(processItem?.key)
                    )}
                    selectedOption={
                      ...
                      >
                        {value?.map((processItem, index) => (
                          <div
                            key={index}
                            className={
                              "w-max px-3 py-2 bg-muted flex items-center justify-between gap-5"
                            }
                          >
                            {
                              processNameEnums[
                                processItem as keyof typeof processNameEnums
                              ]?.label
                            }
                            <Trash2
                              ...
                              onClick={() =>
                                onChange(
                                  value?.filter((item) => item !== processItem)
                                )
                              }
                            />
                          </div>
          ...
          <Card className={"col-span-full p-0 rounded-lg space-y-3"}>
            <div className={"p-3 flex items-center justify-between gap-3"}>
              <CardHeader
                className={
                  "p-0 font-semibold text-base flex flex-row items-center gap-1"
                }
              >
            ...
            {!!getValues("color")?.length &&
              !!getValues("quantity")?.length && (
                <div className={"w-full p-3 grid grid-cols-2 gap-5"}>
                  {getValues("color")?.map((colorValue: string, index) => (
                    <div
                      key={index}
                      className={"col-span-full w-full flex items-center gap-3"}
                    >
                      ...
                      <button
                        className={"p-2 hover:bg-muted/70 rounded-full"}
                        onClick={() =>
                          onHandleRemoveColorQuantity(
                            colorValue,
            ...
            {colorQuantityPayload?.isFormEnabled && (
              <>
                ...
                <div
                  className={
                    "w-full p-3 grid grid-cols-3 place-content-center place-items-center gap-3"
                  }
                >
                  <div className={"col-span-full md:col-span-1 space-y-1"}>
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        onHandleChangeText("color", event?.target.value)
                      }
                    />
                  </div>
                  <div className={"col-span-full md:col-span-1 space-y-1"}>
                  ...
                  <div
                    className={"col-span-full md:col-span-1 flex justify-end"}
                  >
                    <Button
                      className={"cursor-pointer disabled:cursor-not-allowed"}
                      variant={"outline"}
                      disabled={
                        !colorQuantityPayload.color ||
                        !colorQuantityPayload.quantity
                      }
            ...
            render={({ field: { value, onChange } }) => (
              <FormItem className={"col-span-full flex flex-col gap-1.5"}>
                <div className={"flex items-center justify-between gap-3"}>
                  ...
                  <OptionSelector
                    options={Object.values(skippedStagesEnums)?.filter(
                      (skippedStage) => !value?.includes(skippedStage?.key)
                    )}
                    selectedOption={
                      <CirclePlus size={20} className={"cursor-pointer"} />
                    }
                      ...
                      >
                        {value?.map((skippedStage, index) => (
                          ...
                          >
                            {
                              skippedStagesEnums[
                                skippedStage as keyof typeof skippedStagesEnums
                              ]?.label
                            }
                            <Trash2
                              className={"cursor-pointer"}
                              ...
                              onClick={() =>
                                onChange(
                                  value?.filter((item) => item !== skippedStage)
                                )
                              }
                            />
                          </div>
        ...
        <div className={"w-full flex flex-col gap-5"}>
          ...
          <div className={"flex items-center justify-end gap-5"}>
            <Button
              type={"submit"}
              disabled={
                isLoading || isSubmitting || !isValid || fabricList?.isLoading
              }
              className={cn(
