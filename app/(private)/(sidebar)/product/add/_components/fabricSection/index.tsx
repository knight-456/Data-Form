...
const FabricHeader = () => {
  return (
    <div className={"space-y-5 mb-5"}>
      <DialogHeader>
        <DialogTitle className={"text-xl"}>{"Add Fabric"}</DialogTitle>
      </DialogHeader>
      <div className={"-mx-5"}>
  ...
  return (
    <Card className={"col-span-full p-5 rounded-lg space-y-3"}>
              ...
              <DropdownMenuGroup>
                {fabricList?.data?.map((item: any, index: number) => (
                  <DropdownMenuItem key={index}>
                    {
                      fabricNameEnums[
                        item?.fabricName as keyof typeof fabricNameEnums
                      ]?.label
                    }
                  </DropdownMenuItem>
                ))}
      ...
      <button
        type={"button"}
        className={
          "w-full flex items-center justify-center gap-2 py-2 border-2 border-dashed border-muted rounded-md"
        }
