...
const ChinaFabric = () => {
  ...
  const majorFabricOptions = useMemo(() => {
    ...
    const availableFabrics = fabricList?.data
      ?.map((fabricItem: any) => ({
        label:
          fabricNameEnums[
            fabricItem?.fabricName as keyof typeof fabricNameEnums
          ]?.label,
      ...
      {!!getValues("isChinaFabricPresent") && (
        <FormField
          ...
          render={({ field: { value, onChange } }) => (
            <FormItem
              className={
                "col-span-full md:col-start-1 md:col-span-1 w-full flex flex-col md:flex-row md:items-center md:justify-start gap-5"
              }
            >
                      ...
                      <OptionSelector
                        ...
                        selectedOption={
                          <Button
                            variant={"ghost"}
                            className={
                              "w-fit items-center justify-between gap-3 bg-muted text-muted-foreground capitalize"
                            }
                          >
                            {"Add More"}
                    ...
                    <OptionSelector
                      ...
                      selectedOption={
                        <Button
                          variant={"ghost"}
                          className={
                            "flex-1 items-center justify-between gap-3 bg-muted text-muted-foreground capitalize"
                          }
                        >
                          {"Select China Fabric"}
                          <ChevronsUpDown size={16} />
            ...
            <FormControl>
              <OptionSelector
                ...
                selectedOption={
                  ...
                  >
                    {fabricNameEnums[value as keyof typeof fabricNameEnums]
                      ?.label ?? "None"}
                    <ChevronsUpDown size={16} />
                  </Button>
                }
